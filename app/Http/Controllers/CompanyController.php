...
class CompanyController extends Controller
{
    ...
    /**
     * Function to update province image
     * @param void
     * @return array
     */
    public function updateProvinceImage(Request $request)
    {
        ...
        $validation = Validator::make(
		    ...
		    array(
		        'provinceImage' => array('required')
		    ),
		    array(
		        'provinceImage.required' => 'Please select image to upload'
		    )
		);
			    ...
			    if( ( $fileType == 'image/jpeg' || $fileType == 'image/jpg' || $fileType == 'image/png' ) && $fileSize <= 3000000 )     // 3 MB = 3000000 Bytes
			    {
			        ...
			        if( $companyImage->move( $destinationPath, $fileNewName ) )
			        {
			        	...
			        	if( $user->save() )
			        	{
			        		$response['errCode']    = 0;
		        			$response['errMsg']     = 'Image uploaded successfully';
		        			$response['imgPath']    = url('/images/province/' . $fileNewName);
			        	}
			        	else
			        	{
			        		$response['errCode']    = 2;
